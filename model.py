...
from tensorflow.keras.layers import BatchNormalization, Dropout, Conv2D, TimeDistributed
from tensorflow.keras.layers import Lambda, Flatten, Activation, Dense, Input, ConvLSTM2D
...
#     """
    ...
    point_cloud_shape = tf.shape(point_cloud_series)
    batch_size = point_cloud_shape[0]
    time_step = point_cloud_shape[-3]
...
def get_model(graph_path,
    ...
    static_graph = tf.constant(graph, dtype=tf.int32)

    # 5 conv layers

    # 4 stacking conv layers
    net1 = T_edge_conv(main_input, graph=static_graph, kernel=kernels[0], k=k)
    net2 = T_edge_conv(net1, graph=static_graph, kernel=kernels[1], k=k)
    net3 = T_edge_conv(net2, graph=static_graph, kernel=kernels[2], k=k)
    net4 = T_edge_conv(net3, graph=static_graph, kernel=kernels[3], k=k)
    ...
    net = T_edge_conv(net, graph=static_graph, kernel=kernels[4], k=k)
    ...
    output_layer = net
    model = keras.models.Model(main_input, output_layer)
